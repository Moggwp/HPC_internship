...
# inputs
peakpower = 1000 # 1 kW of installed capacity (can be scaled later)
target_year = 2023
...
co2 = co2.resample("15min").mean().bfill()
...
data['Emissões Indiretas de CO2'] = data["Enet Emissions"] * data["Intensidade Carbónica"]

fig, ax = plt.subplots()
ax2 = ax.twinx()
#ax.plot(data.index, data['PV'], label = 'PV')
ax.plot(data.index, -data['Emissões Indiretas de CO2'], label = 'Emissões indiretas (gCO2)', color = 'red')
#ax.plot(data.index, data['Consumos'], label = 'Consumos')
#ax2.plot(data.index, data['Pnet'], label = 'Pnet')
ax2.plot(data.index, data['Enet Emissions'], label = 'Energia Importada (kWh)', color = 'green')
ax.legend()
ax2.legend()
ax.set_ylabel("Emissões indiretas (gC02)")
ax2.set_ylabel("Energia Importada (kWh)")
plt.show()
